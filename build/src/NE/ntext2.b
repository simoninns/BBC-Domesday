...
/**
         NE.NTEXT2 - PAGE DISPLAY ROUTINES FOR NATIONAL ESSAY
         ----------------------------------------------------
         ...
         31.7.87  2        PAC     Fix chained.essay bug
**/
...
/**
         G.NE.DISPLAYPAGE - routine to output text page to screen,
         --------------------------------------------------------
                            printer or floppy
                            ----------------
...
$(
   ...
   if (g.ne.s!m.ne.firstinbuff = m.ne.invalid) |
      (g.context!m.page.no < g.ne.s!m.ne.firstinbuff) |
      (g.context!m.page.no >= (g.ne.s!m.ne.firstinbuff +
       g.ne.s!m.ne.max.pages))
   then read.data()
  ...
  // set up byte offset from start of G.ne.buff to the page we want
  //
  tptr :=  m.ne.photo.data.size + 
          (g.context!m.page.no - g.ne.s!m.ne.firstinbuff)*m.sd.pagelength

  if ((g.ne.buff%tptr) & #x80) ~= 0  // extra code added 12.8.86 PAC
  then
  $( propspaced     := FALSE
     g.ne.buff%tptr := g.ne.buff%tptr & #x7F // mask top bit
  $)  
  ...
  $)
  if result then
  g.sc.opage( g.ne.buff,
              m.sd.invalid,
              (g.ne.s!m.ne.max.pages)*m.sd.pagelength,
              m.sd.invalid,
              tptr,
              m.sd.invalid,
              propspaced,
              type,
              g.ne.s!m.ne.pagebuff )

  updatemenu()

  unless propspaced                
  do g.ne.buff%tptr := (g.ne.buff%tptr) | #x80 // reset top bit
...
$(
   ...
   LET bytes.to.read = ?
   LET max.pg  = g.ne.s!m.ne.max.pages
   LET num.pg  = g.ne.s!m.ne.nopages
   LET handle  = ?
   LET ptr     = VEC 1
   ...
   handle := g.ne.s!m.ne.text.is.data2 -> g.ne.s!m.ne.D2.handle,
   ...
   startpage := g.context!m.page.no - (g.ne.s!m.ne.max.pages/3)
   ...
   G.ut.add32( g.ne.s+m.ne.itemaddress,ptr) // now set pointer into DATA1/2
   ...
   g.dh.read( handle,
              ptr,
              g.ne.buff + m.ne.photo.data.size/bytesperword, 
              bytes.to.read )

   g.ne.s!m.ne.firstinbuff := startpage
$)
...
$(
   ...
   if (g.context!m.page.no = 1) & (g.ne.s!m.ne.notitles <=1)
   then
   $( // check first page is not a photo
      unless ( (g.ne.s!m.ne.type = m.ne.picessay) &
               (g.ut.unpack16(g.ne.buff,0) <= 0) )
      do
      $(
   ...
   if (g.context!m.page.no = g.ne.s!m.ne.nopages)
   then        
   $(
      let i = 0
      ...
      $( // check if any more picture pages after this textpage
         ...
         $(
            test g.ut.unpack16(g.ne.buff,i*m.ne.rsize) = g.context!m.page.no
            then more := true
...
$(
   ...
   $(
      drawbluebar()
      test (g.ne.s!m.ne.notitles <= 1) & (g.context!m.page.no = 1)
      then
      $( // show article title on first textpage if no Contents page
         ...
         g.sc.oprop(g.ne.s+m.ne.titlebuff)
         len := 13
      $)
      else len := 16   
...
$(
   ...
   if (g.ne.s!m.ne.notitles <= 1) & (g.context!m.page.no = 1)
   then
   $(
      buff%0 := m.ne.title.size
      for i=1 to 30 do buff%i := (g.ne.s+m.ne.titlebuff)%i   
   ...
   $(
      ...
      $(
         ...
         else $( no := g.ne.s!m.ne.nopages ; p := 37 $)
...
$(
   ...
   IF (essay.no = 3) | ((essay.no = 2) & three.invalid)
   THEN IF (g.ne.s!m.ne.nopages = g.context!m.page.no) // test last page
